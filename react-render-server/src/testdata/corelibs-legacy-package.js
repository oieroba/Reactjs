...
(function($) {
    ...
    function init(opts) {
        ...
        var clickRow = function(e) {
                // For accessibility, the link will often have href="#" but we
                // don't want to navigate to the target in that case
 ...
 * enough to warrant its own documentation.
    ...
    This fork also automatically sets ARIA attributes on the dropdown toggle
    and menu elements.
 */
  ...
  * ========================= */
  ...
  var toggle = '.dropdown-toggle'
    , Dropdown = function ( element, option ) {
        // Add some initial accessibilty properties
        // The toggle $(element) lets screen readers know it has a popup
        $(element).attr('aria-haspopup', true)
        // The menu is assigned a role of "menu" and has an initial expanded
        // state of false
        var $menu = $(element).parent().find('.dropdown-menu')
        $menu.attr('role', 'menu')
  ...
  , open: function () {
      ...
      $this.parent().find('.dropdown-menu').attr('aria-expanded', true)
  ...
  , close: function () {
      ...
      $this.parent().find('.dropdown-menu').attr('aria-expanded', false)
  ...
  function clearMenus(ev) {
    $(toggle).each(function(idx, el) {
        if ($(el).hasClass('no-auto-close') ||
            $(ev.target).closest($(el).parent()).length > 0) {
            // If user clicks on the dropdown element or a child of one,
            // don't close that dropdown
            return;
        }
        $(el).trigger('close')
            .parent().removeClass('open')
  ...
  $(function () {
    $('html').on('click.dropdown.data-api', clearMenus)
        .on('focusin.dropdown.data-api', clearMenus)
  })
...
//
// Since en-US is the default language used by the moment library, we
// don't need to have any content here.  This file is just a
// placeholder indicating the user wants US English.
//
// We need this file because inject_i18n_strings causes
// __language__.js to point to en.js for unsupported languages (such
// as 'boxes'), so en.js must always exist in this directory.
//
// Maybe the right fix is to change inject_i18n_strings.py to point
// somewhere else if en.js doesn't exist, but this fix is easier.
});
